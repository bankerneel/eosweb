...
export class ProducerComponent implements OnInit, OnDestroy{
  ...
  options = {
    layers: [
       tileLayer('https://cartodb-basemaps-{s}.global.ssl.fastly.net/dark_all/{z}/{x}/{y}{r}.png', { maxZoom: 18, attribution: 'AladinWeb' })
    ],
    zoom: 1,
    center: latLng(0, 0)
  };
  ...
  getData(){
      this.spinner = true;
  		let producers = this.http.get(`/api/custom/get_table_rows/alaio/alaio/producers/500`)
